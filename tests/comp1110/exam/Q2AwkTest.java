...
public class Q2AwkTest {
    ...
    public void testJavadocExample() {
        ...
        try {
            ...
            String expected = getExpected(new Integer[]{1, 2, 39, 6});
            String actual = Files.readString(Paths.get(outputFilename));
            assertEquals("incorrect result returned from extractColumnCSV(\"" + inputFilename + ", 2, " + outputFilename + "\", " + true, expected, actual);
        } catch (IOException e) {
            fail(e.getMessage());
        }
    }
    ...
    private <T> void checkColumn(int column, boolean sort, T[] rows) {
        try {
            ...
            String expected = getExpected(rows);
            String actual = Files.readString(Paths.get(OUTPUT_FILENAME));
            assertEquals("incorrect result returned from extractColumnCSV(\"" + INPUT_FILENAME + "\", " + column + ", \"" + OUTPUT_FILENAME + "\", " + sort, expected, actual);
        } catch (IOException e) {
            fail("Unexpected IOException: " + e.toString());
        }
    }

    <T> String getExpected(T[] rows) {
